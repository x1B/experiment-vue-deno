const VUE_ROOT_URL = 'https://raw.githubusercontent.com/x1B/vue/experiment/deno-ssr';
...
export async function loadVue() {
    const vueModule: { default: any } = await import(`${VUE_ROOT_URL}/dist/vue.esm.js`);
...
export async function render(vm: any): Promise<String> {
  ...
  await Promise.all([ loadVue(), loadRenderer() ]);
  return new Promise((resolve, reject) => {
    global.renderVueComponentToString(vm, (err: any, html: string) => 
