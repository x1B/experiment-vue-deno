...
if (process.env.NODE_ENV !== "production") {
  ...
  var warnReservedPrefix = function (target, key) {
    warn(
      'Property "' + key + '" must be accessed with "$data.' + key +
        '" because ' +
...
function bindDynamicKeys(baseObj, values) {
  for (var i = 0; i < values.length; i += 2) {
    ...
    if (typeof key === "string" && key) {
      ...
      process.env.NODE_ENV !== "production" && key !== "" && key !== null
    ) {
...
function _createElement(
  ...
  if (typeof tag === "string") {
    ...
    if (config.isReservedTag(tag)) {
      ...
      if (
        process.env.NODE_ENV !== "production" && isDef(data) &&
        isDef(data.nativeOn)
      ) {
        warn(
...
function createElement$1(tagName, vnode) {
  ...
  if (
    vnode.data && vnode.data.attrs && vnode.data.attrs.multiple !== undefined
  ) {
...
function createPatchFunction(backend) {
  ...
  function removeAndInvokeRemoveHook(vnode, rm) {
    if (isDef(rm) || isDef(vnode.data)) {
      ...
      if (
        isDef(i = vnode.componentInstance) && isDef(i = i._vnode) &&
        isDef(i.data)
      ) {
    ...
    if (isDef(tag)) {
      if (isDef(children)) {
        ...
        if (!elm.hasChildNodes()) {
          ...
          if (
            isDef(i = data) && isDef(i = i.domProps) && isDef(i = i.innerHTML)
          ) {
            if (i !== elm.innerHTML) {
              /* istanbul ignore if */
              if (
...
function updateDOMProps(oldVnode, vnode) {
  ...
  for (key in props) {
    ...
    if (key === "value" && elm.tagName !== "PROGRESS") {
      ...
      key === "innerHTML" && isSVG(elm.tagName) && isUndef(elm.innerHTML)
    ) {
...
function updateStyle(oldVnode, vnode) {
  ...
  if (
    isUndef(data.staticStyle) && isUndef(data.style) &&
...
function parseHTML(html, options) {
  ...
  while (html) {
    ...
    if (html === last) {
      ...
      if (
        process.env.NODE_ENV !== "production" && !stack.length && options.warn
      ) {
        options.warn(
  ...
  parseHTML(template, {
    ...
    chars: function chars(text, start, end) {
      ...
      if (text) {
        ...
        if (child) {
          if (
            process.env.NODE_ENV !== "production" && options.outputSourceRange
          ) {
            child.start = start;
            child.end = end;
          }
